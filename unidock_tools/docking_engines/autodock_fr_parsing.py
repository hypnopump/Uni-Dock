...
def parse_dlg_file(dlg_file_name):
    ...
    for dlg_line_idx, dlg_line in enumerate(dlg_lines):
        if 'Refining results ...' in dlg_line:
            docked_cluster_starting_line_idx = dlg_line_idx
        elif 'Clustering information' in dlg_line:
    ...
    num_docked_conformations = len(docked_cluster_info_nested_list)

    binding_free_energy_array = np.zeros((num_docked_conformations), dtype=np.float32)
    cluster_size_array = np.zeros((num_docked_conformations), dtype=np.int32)
    for docked_conf_idx in range(num_docked_conformations):
        docked_cluster_line_split_list = docked_cluster_info_nested_list[docked_conf_idx]
        binding_free_energy_array[docked_conf_idx] = np.float32(docked_cluster_line_split_list[1])
        cluster_size_array[docked_conf_idx] = np.int32(docked_cluster_line_split_list[4])
    #####################################################################################
...
def autodock_parsing_process(reference_sdf_file_name,
    ...
    reference_mol, _ = utils.prepare_covalent_ligand_mol(reference_mol)

    docked_sdf_file_name_prefix = os.path.basename(dlg_file_name).split('.')[0].replace('_adfr_summary', '')

    parsed_docking_dlg_info_dict = parse_dlg_file(dlg_file_name)
    ...
    binding_free_energy_array = parsed_docking_dlg_info_dict['binding_free_energy']
    cluster_size_array = parsed_docking_dlg_info_dict['cluster_size']
...
class AutoDockFRParsing(object):
    def __init__(self,
